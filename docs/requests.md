...
| 7 | "visualization app structure before built, road maps and more … visualize plans further … more ways to improve my prompts … a library that saves useful assets … tags and keywords we can run past clients linked briefs" | 2026-09-18 | UI review | Designed — `docs/visualize-and-library.md`: Map/Roadmap/Storyboard lenses, Prompt Ledger + Lint + effective view, the Library with client shortlists |
| 8 | "Add both those" — the mark-up set in both products | 2026-09-18 | UI review | Recorded — LodiStudios `docs/webpro-markup.md`; studio README roadmap note; overlay built once, vendored into each |
| 9 | "auto sort organize tag and library them as they're created automatically on your end say the drones from DGM and roamers from JBN" | 2026-09-18 | UI review | Designed — auto-catalog step at the end of every run; owner's names win via the glossary; retroactive harvest of DGM / JBN / greenflash |
| 10 | Review notes for each push live in `docs/reviews/` — the Mac session reads them after `git pull` | 2026-09-18 | build | Convention — first: `2026-09-18-v0.1-shell.md` |
| 11 | "Can this still use my ssh keys for other droplets to SSH in via the LodiTerminal?" | 2026-09-18 | build | Partly: importing existing RSA/ECDSA keys works; **ed25519 keys cannot be used with the mbedTLS build** (the owner's Termius key is ed25519). Either install the app's own P-256 key on each host (the app should offer "install my key on this host" once one route works), or rebuild libssh2 on OpenSSL at v0.2 if importing ed25519 keys matters. Decide at v0.2. |
